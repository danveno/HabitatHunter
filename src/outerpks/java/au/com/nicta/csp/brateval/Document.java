...
/**
 * Document class, it includes implementations for entity and relation
 * comparisons
 * 
...
public class Document {
	...
	public List<Entity> findAllEntitiesSpanOverlap(Entity e) {
		...
		for (Entity e1 : entities.values()) {
			if (Entity.entityComparisonSpanOverlap(e, e1)
					|| Entity.entityComparisonSpanOverlap(e1, e)) {
				if (!e.equals(e1)) {
	...
	/**
	 * find exactly matched entity which shares the same entity type, name and location
	 * 
	 * @param e
	 * @return
	 */
	...
	/**
	 * the two entity must be the same type and have span overlap
	 * @param e
	 * @return
	 */
	public Entity findEntityOverlapC(Entity e) {
		for (Entity e1 : entities.values()) {
			if (e1.getType().equals(e.getType())
					&& Entity.entityComparisonSpanOverlap(e, e1)
					&& !e.equals(e1)) {
				return e1;
			}
		}
	 ...
	 * @param relation
	 * @param d
	 * @return Return the relation in the matching document or null if no
	...
	public Relation findRelationOverlap(Relation relation) {
		for (Relation rd : getRelations()) {
			...
			if (relation.getRelationType().equals(rd.getRelationType())
					&& (Entity.entityComparisonOverlap(relation.getEntity1(),
							rd.getEntity1()) && Entity.entityComparisonOverlap(
							relation.getEntity2(), rd.getEntity2()))) {
				// Compare entities
				return rd;
			}
		}
	...
	public Relation findRelation(Relation relation) {
		for (Relation rd : getRelations()) {
			// Compare relation type
			if (relation.getRelationType().equals(rd.getRelationType())
					&& ((Entity.entityComparison(relation.getEntity1(),
							rd.getEntity1()) && Entity.entityComparison(
							relation.getEntity2(), rd.getEntity2()))
					// Order does not matter
					|| (Entity.entityComparison(relation.getEntity1(),
							rd.getEntity2()) && Entity.entityComparison(
							relation.getEntity2(), rd.getEntity1())))) {
				// Compare entities
				return rd;
			}
		}
	...
	public boolean hasEntityRelation(Entity e) {
		for (Relation r : getRelations()) {
			if (Entity.entityComparison(e, r.getEntity1())
					|| Entity.entityComparison(e, r.getEntity2())) {
				return true;
			}
		}
