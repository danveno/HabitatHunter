...
public class Entity {
	...
	public Entity(String id, String type, LinkedList<Location> l,
	...
	public String toString() {
		...
		if (l.size() > 0) {
			locations.append(l.get(0).getStart()).append(" ")
					.append(l.get(0).getEnd());

			for (int i = 1; i < l.size(); i++) {
				locations.append(";").append(l.get(i).getStart()).append(" ")
	...
	/**
	 * match entity type, name, and location
	 * 
	 * @param e1
	 * @param e2
	 * @return
	...
	public static boolean entityComparison(Entity e1, Entity e2) {
		if (e1.getType().equals(e2.getType())
				&& e1.getString().toLowerCase()
						.equals(e2.getString().toLowerCase())) {
	...
	private static boolean entityCompareOverlap(Entity e1, Entity e2) {
		for (Location l1 : e1.getLocations()) {
			for (Location l2 : e2.getLocations()) {
				if ((l1.getStart() >= l2.getStart() && l1.getStart() <= l2
						.getEnd())
						|| (l1.getEnd() >= l2.getStart() && l1.getEnd() <= l2
								.getEnd())) {
					return true;
				}
			}
