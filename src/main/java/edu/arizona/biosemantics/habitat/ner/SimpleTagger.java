...
public class SimpleTagger {
	protected static Logger logger = MalletLogger.getLogger(SimpleTagger.class
			.getName());
	...
	public static class SimpleTaggerSentence2FeatureVectorSequence extends Pipe {
		...
		protected static final long serialVersionUID = -2059308802200728625L;
	...
	protected static final CommandOption.Double gaussianVarianceOption = new CommandOption.Double(
			SimpleTagger.class, "gaussian-variance", "DECIMAL", true, 10.0,
			"The gaussian prior variance used for training.", null);

	protected static final CommandOption.Boolean trainOption = new CommandOption.Boolean(
			SimpleTagger.class, "train", "true|false", true, false,
			"Whether to train", null);

	protected static final CommandOption.String testOption = new CommandOption.String(
			SimpleTagger.class,
			"test",
	...
	protected static final CommandOption.File modelOption = new CommandOption.File(
			SimpleTagger.class,
			"model-file",
			"FILENAME",
			true,
	...
	protected static final CommandOption.File rsOption= new CommandOption.File(
			SimpleTagger.class,
			"rs-file",
			"FILENAME",
			true,
			null,
			"The filename for saving predicted results.",
			null);

	protected static final CommandOption.Double trainingFractionOption = new CommandOption.Double(
			SimpleTagger.class, "training-proportion", "DECIMAL", true, 0.5,
			"Fraction of data to use for training in a random split.", null);

	protected static final CommandOption.Integer randomSeedOption = new CommandOption.Integer(
			SimpleTagger.class,
			"random-seed",
			"INTEGER",
			true,
	...
	protected static final CommandOption.Boolean featureInductionOption = new CommandOption.Boolean(
			SimpleTagger.class, "feature-induction", "true|false", true, false,
			"Whether to perform feature induction during training", null);

	protected static final CommandOption.Integer numThreads = new CommandOption.Integer(
			SimpleTagger.class, "threads", "INTEGER", true, 1,
			"Number of threads to use for CRF training.", null);

	protected static final CommandOption.List commandOptions = new CommandOption.List(
			...
			new CommandOption[] { gaussianVarianceOption, trainOption,
					...
					modelOption, rsOption, randomSeedOption, ordersOption,
					forbiddenOption, allowedOption, defaultOption,
					viterbiOutputOption, connectedOption, weightsOption,
					continueTrainingOption, nBestOption, cacheSizeOption,
					includeInputOption, featureInductionOption, numThreads });
	...
	public static CRF train(InstanceList training, InstanceList testing,
		...
		if (numThreads.value > 1) {
			...
			if (featureInductionOption.value) {
				throw new IllegalArgumentException(
						"Multi-threaded feature induction is not yet supported.");
			} else {
				boolean converged;
				for (int i = 1; i <= iterations; i++) {
					...
					if (viterbiOutputOption.value && i % 10 == 0)
						new ViterbiWriter("", new InstanceList[] { training,
								testing },
			...
			if (featureInductionOption.value) {
				crft.trainWithFeatureInduction(training, null, testing, eval,
						iterations, 10, 20, 500, 0.5, false, null);
			} else {
				boolean converged;
				for (int i = 1; i <= iterations; i++) {
					converged = crft.train(training, 1);
	 ...
	 * @param k
	 *            the number of answers to return
	 * @return array of the k highest-scoring output sequences with scores
	 */
	public static Object[] applyWithWeights(Transducer model, Sequence input, int k) {
		Sequence[] answers;
		...
		if (k == 1) {
			...
			MaxLatticeDefault lattice = new MaxLatticeDefault(model, input,
					null, cacheSizeOption.value());
			List<SequencePairAlignment<Object,Object>> bestKAlignments = lattice.bestOutputAlignments(k);
			ArrayList<Sequence<Object>> ret = new ArrayList<Sequence<Object>>(k);
			for (int i = 0; i < k; i++){
				ret.add ( bestKAlignments.get(i).output());
				weights[i] = bestKAlignments.get(i).getWeight();
				System.out.print(weights[i]+" ");
			}
			System.out.println();
			answers = ret.toArray(new Sequence[0]);
		}
		return new Object[]{answers, weights};
	}
	 ...
	 *            <dd>Create sparse, some dense (using a heuristic), or dense
	 *            features on transitions. Default is <code>some-dense</code>.</dd>
	...
	public static void main(String[] args) throws Exception {
		...
		if (trainOption.value) {
			...
			if (eval != null)
				...
				System.out.println("it's testing now "+includeInputOption.value()+" "+nBestOption.value+" "+cacheSizeOption.value());
				boolean includeInput = includeInputOption.value();
				FileWriter rsFw = new FileWriter(rsOption.value);
